...
var this_server = document.domain;
var s_account="hboprod" 
...
s.linkInternalFilters="javascript:,hbo.com"
...
/*Kenshoo Click ID */
    if(s.getQueryParam('K_CLICKID') != "") {
        s.eVar43 = s.getQueryParam('K_CLICKID');
    }
    /*Kenshoo Keyword ID */
    if(s.getQueryParam('tracking_ID') != "") {
        s.eVar41 = s.getQueryParam('tracking_ID');
    }
	...
	s.detectRIA('s_ria','prop29','','15','','');

	/* Set Page View Event */
	s.events=s.apl(s.events,'event1',',',2)

	/* Set Time Parting Variables - SAMPLE EST */
	s.prop26=s.TimeParting('h','-5'); // Set hour 
	s.prop27=s.TimeParting('d','-5'); // Set day 
	s.prop28=s.TimeParting('w','-5'); // Set weekday v.Weekend

	/* Copy props to eVars */
	if(s.pageName&&!s.eVar1) s.eVar1=s.pageName;
...
/*
* Plugin Utility: p_fo - only fire plugin on first do_plugins execution
*/
